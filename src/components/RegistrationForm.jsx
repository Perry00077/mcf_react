...
export default function RegistrationForm() {
  ...
  async function handleSubmit(event) {
    ...
    if (error) {
      setStatus({
        type: "error",
        message: error.message || dictionary.genericError,
      });
      setLoading(false);
      return;
    }
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <Field label={dictionary.message} className="md:col-span-2">
              <Textarea
                ...
                value={form.message}
                onChange={handleChange}
                placeholder="Tell us about your arrival or accommodation preferences."
              />
            </Field>
          </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm leading-6 text-slate-400">
              Comments in this component explain the optional email trigger and
              the anti-spam honeypot.
            </p>
            <Button
              type="submit"
              size="lg"
